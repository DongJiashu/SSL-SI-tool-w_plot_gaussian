"""
Phoneme Gaussian Model Training and Evaluation System

Usage Examples:
1. Train new models:
   python xx.py --train-csv train_data.csv --phoneme-combinations ɪ_t͡ʃ_ɪ ɪ_s_ɪ
...
3. Train and evaluate in one go:
   python xx.py --train-csv train_data.csv --eval-csv new_samples.csv --phoneme-combinations ɪ_t͡ʃ_ɪ
...
--phoneme-combinations: Phoneme patterns to analyze (space separated)
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description='Phoneme Gaussian Model Training and Evaluation')
    ...
    parser.add_argument('--phoneme-combinations', type=str, nargs='+', 
                       help='Phoneme combinations to analyze (required for training)')
...
def prepare_data(df: pd.DataFrame, phoneme_combinations: List[str]) -> Dict[str, pd.DataFrame]:
    """Prepare data by filtering and organizing by phoneme combinations"""
    plot_data = {}
...
def train_gaussian_models(plot_data: Dict[str, pd.DataFrame]) -> Dict[str, Dict[str, Any]]:
    """
    Train Gaussian models for each phoneme combination and feature category
...
def evaluate_sample(sample_data: pd.DataFrame, models: Dict[str, Dict[str, Any]], 
    ...
    # Get the actual phoneme combination from the sample data
    sample_phonemes = sample_data['phoneme_combination'].unique()
    if len(sample_phonemes) > 0:
        actual_phoneme = sample_phonemes[0]
    else:
    ...
    for phoneme, phoneme_models in models.items():
        # Only evaluate models that match the sample's phoneme combination
    ...
    # Store only the actually evaluated phoneme combinations
    results['phoneme_combinations'] = evaluated_phonemes
...
def combine_phoneme_plots(models: Dict[str, Dict[str, Any]], output_dir: str, 
    ...
    """Combine plots for each phoneme combination and add feedback text below"""
    ...
    for phoneme in phonemes:
        ...
        if eval_results:
            ...
            for sample_name, results in eval_results.items():
                if phoneme in results.get('phoneme_combinations', []):
...
def main():
    ...
    if models is None and args.train_csv:
        if not args.phoneme_combinations:
            print("Error: Phoneme combinations are required for training")
            return
        ...
        # Prepare data
        plot_data = prepare_data(train_df, args.phoneme_combinations)
        if not plot_data:
        ...
        try:
            ...
            for base_name in unique_samples:
                try:
                    ...
                    print(f"Phoneme Combination: {results['phoneme_combinations'][0] if results['phoneme_combinations'] else 'None'}")
