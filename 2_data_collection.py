...
def process_phoneme_combinations(folder_a, folder_b, phoneme_combinations):
    """
    Process phoneme combinations from TextGrid files and extract corresponding features from npy files.
    ...
    Args:
        ...
        phoneme_combinations (list): List of phoneme combinations to search for (e.g., ['ɪ_ʃ_t', 'ɪ_s_t', 'i_t͡ʃ_t'])
    ...
    for npy_path in npy_files:
        ...
        try:
            ...
            # Process each phoneme combination
            for combo in phoneme_combinations:
                phonemes = combo.split('_')
                combo_results = find_phoneme_combination(
                    tg, npy_data, base_name, wav_path, phonemes, combo
                )
                if combo_results:
                    print(f"  Found {len(combo_results)} instances of {combo}")
...
def find_phoneme_combination(tg, npy_data, base_name, wav_path, target_phonemes, combo_name):
    """
    Find a specific phoneme combination in TextGrid and extract corresponding features.
    ...
    Args:
        ...
        combo_name: Name of the phoneme combination
    ...
    # Find all occurrences of the phoneme combination
    for i in range(len(phones_tier) - len(target_phonemes) + 1):
        # Check if current sequence matches target phonemes
        match = True
        for j, target_phoneme in enumerate(target_phonemes):
            if i + j >= len(phones_tier):
                match = False
                break
            current_phoneme = phones_tier[i + j]['text'].strip().lower()
        ...
        if match:
            ...
            phoneme_end = phones_tier[i + len(target_phonemes) - 1]['xmax']
            ...
            # 2. Phoneme combination time region
            phoneme_combo_features = extract_feature_values(npy_data, phoneme_start, phoneme_end)
            
            # 3. Individual phoneme time regions
            individual_phoneme_features = []
            for j in range(len(target_phonemes)):
                phoneme_start_j = phones_tier[i + j]['xmin']
                phoneme_end_j = phones_tier[i + j]['xmax']
                phoneme_features = extract_feature_values(npy_data, phoneme_start_j, phoneme_end_j)
                individual_phoneme_features.append(phoneme_features)
            
            # 4. Middle phoneme average values (if combination has at least 3 phonemes)
            middle_phoneme_avg = {}
            if len(target_phonemes) >= 3:
                middle_idx = len(target_phonemes) // 2
                middle_start = phones_tier[i + middle_idx]['xmin']
                middle_end = phones_tier[i + middle_idx]['xmax']
                middle_features = extract_feature_values(npy_data, middle_start, middle_end)
                middle_phoneme_avg = calculate_feature_averages(middle_features)
            ...
            result = {
                ...
                # Phoneme combination feature values (all frames)
                'phoneme_combo_feature_values': phoneme_combo_features,
                # Individual phoneme feature values
                'individual_phoneme_features': individual_phoneme_features,
                # Middle phoneme average values
                'middle_phoneme_avg': middle_phoneme_avg,
...
def main():
    parser = argparse.ArgumentParser(description='Process phoneme combinations from TextGrid and npy files')
    ...
    parser.add_argument('phonemes', nargs='+', help='Phoneme combinations to search for (e.g., ɪ_ʃ_t ɪ_s_t i_t͡ʃ_t)')
    
    args = parser.parse_args()
    
    folder_a = args.folder_a
    folder_b = args.folder_b
    phoneme_combinations = args.phonemes
    
    print(f"Folder A (npy files): {folder_a}")
    print(f"Folder B (TextGrid/wav files): {folder_b}")
    print(f"Phoneme combinations: {phoneme_combinations}")
    
    # Process data
    df = process_phoneme_combinations(folder_a, folder_b, phoneme_combinations)
    ...
    if not df.empty:
        ...
        print("No data found for the specified phoneme combinations")
