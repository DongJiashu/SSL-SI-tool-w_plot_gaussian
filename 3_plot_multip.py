...
def parse_arguments():
    ...
    parser.add_argument('phoneme_combinations', type=str, nargs='+', 
                       help='Phoneme combinations to compare (e.g., ɪ_s_t i_t͡ʃ_t) - can specify multiple')
...
def prepare_plot_data(df: pd.DataFrame, phoneme_combinations: List[str]) -> Dict[str, pd.DataFrame]:
    """
    Prepare data for plotting by filtering and organizing by phoneme combinations
    """
    plot_data = {}
    
    for phoneme_combo in phoneme_combinations:
        combo_data = df[df['phoneme_combination'].str.contains(phoneme_combo, na=False)].copy()
        
        if combo_data.empty:
            print(f"Warning: No data found for phoneme combination: {phoneme_combo}")
            available_combos = df['phoneme_combination'].unique()
            print(f"Available phoneme combinations: {available_combos}")
            continue
        
        print(f"Found {len(combo_data)} rows for pattern: {phoneme_combo}")
        print(f"Matching combinations: {combo_data['phoneme_combination'].unique()}")
        
        # Store all data points for this phoneme combination
        plot_data[phoneme_combo] = combo_data
...
def create_combined_plot(plot_data: Dict[str, pd.DataFrame], output_filename: str):
    ...
    category_offsets = {
        'tongue_body': 0,
        'tongue_tip': 1,  # Space between categories #x-limits
        'lips': 2.0 #x-limits 
    }
    
    #colors = ['red', 'blue', 'green', 'orange', 'purple', 'brown', 'cyan', 'magenta', 'yellow', 'black', 'pink', 'gray']
    colors = ['red', 'red', 'green', 'green', 'blue', 'blue', 'cyan', 'magenta', 'yellow', 'black', 'pink', 'gray']
    ...
    for i, (phoneme, phoneme_data) in enumerate(plot_data.items()):
        ...
        for category in categories:
            ...
            for base_name in set(location_dict.keys()) & set(degree_dict.keys()):
                x = location_dict[base_name] + x_offset
                y = degree_dict[base_name]
                
                if phoneme not in plotted_phonemes:
                    scatter = ax.scatter(x, y, color=color, s=100, alpha=0.8, 
                                       label=phoneme, edgecolors='black', linewidth=1,
                                       marker=marker)
            ...
            # Plot unmatched location values
            for base_name in set(location_dict.keys()) - set(degree_dict.keys()):
                x = location_dict[base_name] + x_offset
                y = 0  # Default y value
                
                if phoneme not in plotted_phonemes:
                    scatter = ax.scatter(x, y, color=color, s=80, alpha=0.5, 
                                       label=phoneme, edgecolors='black', linewidth=0.5,
                                       marker=marker)
                    legend_handles.append(scatter)
                    plotted_phonemes.add(phoneme)
                else:
                    ax.scatter(x, y, color=color, s=80, alpha=0.5, 
                             edgecolors='black', linewidth=0.5, marker=marker)
                
                total_points += 1
            
            # Plot unmatched degree values
            for base_name in set(degree_dict.keys()) - set(location_dict.keys()):
                x = 0 + x_offset  # Default x value
                y = degree_dict[base_name]
    ...
    # Create title
    phoneme_names = []
    for phoneme_combo in plot_data.keys():
        parts = phoneme_combo.split('_')
        if len(parts) >= 3:
            middle_phoneme = parts[1]
            phoneme_names.append(f'/{middle_phoneme}/')
    
    title = f"Comparison of Correct Phonemes: {', '.join(phoneme_names)}"
    ...
    for category in categories:
        ...
        # Add ticks for this category range (-0.5 to 0.25)
    ...
    separator_positions = [category_offsets['tongue_tip'] -0.5, category_offsets['lips'] - 0.5] #x-axis limits
    ...
    ax.set_xlim(-0.5, category_offsets['lips'] + 0.5) #x-axis limits
    ...
    print(f"  - Contains {len(plotted_phonemes)} phoneme combinations")
...
def create_individual_plots(plot_data: Dict[str, pd.DataFrame], output_prefix: str):
    ...
    for category in categories:
        ...
        # Set ticks
        ax.set_xticks([-0.5, -0.25, 0.00, 0.25,0.5]) #x-axis limits
...
def main():
    ...
    try:
        ...
        print(f"Unique phoneme combinations: {df['phoneme_combination'].nunique()}")
    ...
    # Prepare plot data
    plot_data = prepare_plot_data(df, args.phoneme_combinations)
    
    if not plot_data:
        print("No valid data found for the specified phoneme combinations")
        return
    ...
    phoneme_str = '_'.join(args.phoneme_combinations).replace(' ', '_').replace('/', '')
